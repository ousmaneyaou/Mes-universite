...
type RestOf<T extends ISession | NewSession> = Omit<T, 'dateDebut' | 'dateFin'> & {
...
export class SessionService {
  ...
  protected convertDateFromClient<T extends ISession | NewSession | PartialUpdateSession>(session: T): RestOf<T> {
    return {
      ...session,
      dateDebut: session.dateDebut?.toJSON() ?? null,
      dateFin: session.dateFin?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSession: RestSession): ISession {
    return {
      ...restSession,
      dateDebut: restSession.dateDebut ? dayjs(restSession.dateDebut) : undefined,
      dateFin: restSession.dateFin ? dayjs(restSession.dateFin) : undefined,
    };
  }
